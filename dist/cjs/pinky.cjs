...
function mapSeries(iterable, asyncFn) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const e of iterable) {
            results.push(yield asyncFn(e, i));
            i++;
        }
        return results;
    });
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const item of iterator_in) {
            ...
            if (running.size >= worker_count) {
                running.delete(yield Promise.race(running.values()));
            }
            // Continue on
            count++;
        }
...
function workerAll(number_of_workers, iterator_in, asyncFn) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const raw_results = yield Promise.all(worker_promises);
        let results = [];
...
function allProps(obj) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const key in promises) {
            results[key] = yield promises[key];
        }
        return results;
    });
}
 ...
 * @throws  {Error}
 */
function waitFor(timeout_ms, condition_fn, { wait_ms = 1000, label = 'condition' /*backoff = 'linear'*/ } = {}) {
    return __awaiter(this, void 0, void 0, function* () {
        let count = 0;
        const start = Date.now();
        const timeout = start + timeout_ms;
        while (timeout > Date.now()) {
            const condition_res = yield Promise.resolve(condition_fn());
            if (condition_res)
                return { total_ms: Date.now() - start, count, result: condition_res };
            yield delay(wait_ms + (count * wait_ms));
            count++;
        }
